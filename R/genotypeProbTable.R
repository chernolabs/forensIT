genotypeProbTable_FM <- function(resQQ,bplot=FALSE,numMarkers=4, freq){
  ...
  for(i in seq_along(bn1$alelFreq)){
    ...
    if(FALSE){
      ...
      aux2    <- genotypeProbs(bn1$alelFreq[[sysName[[i]]]],bn1$alelFreq[[sysName[[i]]]])
